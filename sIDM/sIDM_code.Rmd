...
### Set directory 
dir <- "/Users/justinvanee/Library/Mobile Documents/com~apple~CloudDocs/Documents/brook_trout_postdoc/sIDM"
...
# Data preparation for brook trout analysis (only for viewing: raw data not available due to privacy restrictions) 
...
### Save output 
#save(fit_list, file=file.path(dir, "Outputs", "fit_list.RData"))
...
stan_data$D <- as.matrix(bdiag(stan_data$D))
...
# Compile the model
mod <- cmdstan_model(file.path(dir, "algorithms", "IDM.stan"))
...
## Posterior Prediction of Intensity (only for review, cannot release spatial locations for krigging)
...
## Plot Intensity  (only for review, cannot release spatial locations for krigging)
...
load(file.path(dir, "Outputs", "fit_list.RData"))
...
simulate_wrapper <- function(n) {
  simulate_data(n = n, z_prop = z_prop, p = p)
}
...
# Save output
#save(metrics, file=file.path(dir, "Outputs", "metrics_p5.RData"))
```
...
#load(file=file.path(dir, "Outputs", "metrics_p5.RData"))
...
# Save metrics 
#save(metrics, file=file.path(dir, "Outputs", "metrics_p5_n100_nu.RData"))
...
load(file=file.path(dir, "Outputs", "metrics_p5_n100_nu.RData"))
...
head(DS <- read.table(file.path(dir, "Kery_2024", "data", "amro_obsdetection.csv"),header=T,sep=";",as.is = T))
DS$Distance <- DS$Distance/1000 # Convert to km

# Table including covariates and Oregon2020 counts (particularly the zeros) 
head(sitecovs <- 
       read.table(file.path(dir, "Kery_2024", "data", "amro_sitecovs_simplified.csv"), 
                  header=T,
                  sep=";",
                  as.is = T))

# eBird
head(PC <- read.table(file.path(dir, "Kery_2024", "data", "amro_ebird_simplified.csv"),
                      header=T,
                      sep=";",
                      as.is = T))
...
# Compile the model
mod <- cmdstan_model(file.path(dir, "algorithms", "IDS.stan"))
...
load(file=file.path(dir, "Outputs", "out5A.RData"))
...
head(centroids <- read.table(file.path(dir, "Kery_2024", "data", "env_centroids.csv"),
                             header=T,
                             sep=";",
                             as.is = T))

#load in file with desired crs
polygon.grid <- st_read(file.path(dir, "Kery_2024", "data", "geodata", "grid_1km.shp"))

# mask for study area
mask <- st_read(file.path(dir, "Kery_2024", "data", "geodata", "StudyArea.shp"))
